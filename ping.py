...
def verbose_ping(dest_addr, timeout=2, count=4):
    ...
    for i in range(count):
        ...
        if delay == None:
            print 'ping {}...'.format(dest_addr), 'failed. (Timeout within {} seconds.)'.format(timeout)
        else:
            delay = round(delay * 1000.0, 4)
            print 'ping {}...'.format(dest_addr), 'in {} milliseconds.'.format(delay)
    print('')
...
if __name__ == '__main__':
	if len(sys.argv) > 1:
		for arg in sys.argv[1:]:   #[1:] means start from 1 and go up/forward. THis is to skip argv[0] which is the script name.
			verbose_ping(arg)
			#print arg
