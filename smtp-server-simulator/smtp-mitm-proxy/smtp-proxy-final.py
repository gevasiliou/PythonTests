    ...
    try:
        ...
        while True:
            ...
            if use_starttls and b"STARTTLS" in data.upper():
                ...
                if resp:
                    print(f"{BLUE}REMOTE -> PLC: {resp.decode(errors='ignore').strip()}{RESET}")
...
if __name__ == "__main__":
    ...
    server.listen(5)
    print(f"{GREEN}[*] Proxy Active: 0.0.0.0:{args.listenport} -> {args.remoteserver}:{args.remoteport} (TLS: {args.starttls}){RESET}")
