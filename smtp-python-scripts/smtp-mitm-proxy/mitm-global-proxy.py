...
def pipe(source, destination, label, color, conn_id, show_hex=False):
    try:
        while True:
            ...
            if msg:
                ...
                print(f"{color}[{get_ts()}] [ID:#{conn_id}] {label}:{RESET}")
...
def bridge(client_sock, addr, remote_host, remote_port, force_ssl, show_hex, conn_id):
    ...
    try:
        ...
        # START THE TUNNEL
        t1 = threading.Thread(target=pipe, args=(c_conn, r_conn, "CLIENT->REMOTE", BLUE, conn_id, show_hex), daemon=True)
        t2 = threading.Thread(target=pipe, args=(r_conn, c_conn, "REMOTE->CLIENT", RED, conn_id, show_hex), daemon=True)
