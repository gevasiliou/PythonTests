...
def load_rules():
    ...
    try:
        ...
        if mtime != rules_mtime:
            ...
            # Color-coded reload message (Titan style)
    ...
    try:
        ...
        while True:
            ...
            if b"STARTTLS" in data.upper():
                ...
                threading.Thread(
                    target=pipe,
                    args=(client_ssl, remote_ssl, "DEC (PLC->REMOTE)", BLUE),
                    daemon=True
                ).start()
                pipe(remote_ssl, client_ssl, "DEC (REMOTE->PLC)", RED)
                return

            # normal cleartext SMTP flow before STARTTLS
            remote_sock.sendall(data)
            resp = remote_sock.recv(4096)
            if resp:
                print(f"{RED}REMOTE -> PLC: {resp.decode(errors='ignore').strip()}{RESET}")
